...
export function Products() {
  ...
  return (
    <section className="container-page py-10">
      <div className="mb-8 rounded-lg bg-white p-5 shadow-sm ring-1 ring-zinc-100">
        <div className="mb-5 flex flex-col justify-between gap-4 lg:flex-row lg:items-end">
          <div>
            <p className="text-sm font-semibold uppercase tracking-wide text-blush-500">Tienda</p>
        ...
        <div className="grid gap-3 md:grid-cols-4">
          <input
            ...
            className="h-12 rounded-full border border-zinc-200 bg-white px-4 text-sm outline-none focus:border-blush-300"
          />
          <select value={category} onChange={(event) => updateParam("categoria", event.target.value)} className="h-12 rounded-full border border-zinc-200 bg-white px-4 text-sm outline-none focus:border-blush-300">
            <option value="">Todas las categorias</option>
            {categories.filter((item) => item.name !== "Ofertas").map((item) => <option key={item.name} value={item.name}>{item.name}</option>)}
          </select>
          <select value={subcategory} onChange={(event) => updateParam("subcategoria", event.target.value)} className="h-12 rounded-full border border-zinc-200 bg-white px-4 text-sm outline-none focus:border-blush-300" disabled={!category}>
            <option value="">Todas las subcategorias</option>
            {subcategories.map((item) => <option key={item} value={item}>{item}</option>)}
          </select>
          <select value={sort} onChange={(event) => setSort(event.target.value)} className="h-12 rounded-full border border-zinc-200 bg-white px-4 text-sm outline-none focus:border-blush-300">
